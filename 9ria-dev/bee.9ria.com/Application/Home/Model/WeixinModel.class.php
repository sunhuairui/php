...
class WeixinModel extends Model {
    ...
    public function __construct() {
        if (!empty($_REQUEST['doNotInit'])) return true;

        $content = file_get_contents('php://input');
        !empty($content) || die('这是微信请求的接口地址，直接在浏览器里无效');
        // 记录日志
        addWeixinLog($content, 'received');
    ...
    /* 回复音乐消息 */
    public function replyMusic($media_id, $music_url, $HQ_music_url, $title = '', $description = '') {
        $msg = array();
        $msg['Music'] = array();
