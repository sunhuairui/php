...
class KeywordModel extends Model {
     ...
     */
    public function set($keyword, $addon, $aim_id, $keyword_type = 0, $extra_text = '', $extra_int = '') {
        $data['addon'] = $addon;
        ...
        foreach ($keyword as $key) {
            ...
            $cond = array('keyword'=>$data['keyword'], 'token'=>$data['token']);
            $record = $this->where($cond)->select();
            if ($record) {
     ...
     * 通过关键词获取相关插件信息
     */
    public function get($keyword, $keyword_type = 0) {
        $map = array();
        $map['keyword'] = $keyword;
        $map['token'] = get_token();
     ...
     * 删除关键词
     */
    public function del($addon, $aim_id, $keyword_type = NULL) {
        $data['addon'] = $addon;
        $data['aim_id'] = intval($aim_id);
     ...
     * 按目标条件统一更新关键词
     */
    public function upateByAddon($addon, $aim_id = 0, $save = array()) {
        $map ['addon'] = $addon;
        if ($aim_id) {
