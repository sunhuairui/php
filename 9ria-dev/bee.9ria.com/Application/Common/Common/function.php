...
defined('ONETHINK_ADDON_PATH') or define('ONETHINK_ADDON_PATH', SITE_PATH . '/Addons/');
 ...
 */
function wx_addons_url($url, $param = array()) {    
    $url = parse_url($url);
    ...
    $result = _PHP_FILE_.'?s=/addon/';
    $result .= ucfirst($addons) . '/' . ucfirst($controller) . '/' . $action;
...
function addons_url($url, $param = array()) {
    ...
    $addon_url = U('Home/Addons/execute', $params);
    \Think\Log::record($addon_url, \Think\Log::ERR);
    return str_replace('wx', 'index', $addon_url);
}
...
/**
 * 状态参数处理
 * $string 分析枚举类型字段值 格式 a:名称1,b:名称2
 * $keys  键位
 * auth@changzhengfei
 */
function get_status_value($string,$keys){
    $array=parse_field_attr($string);
    if(!isset($keys)){
        return false;
    }
    if($keys=='0' or $keys){
...
function safe($text, $type = 'html') {
    ...
    // 内容等允许HTML的格式
    $html_tags = $base_tags . '<meta><ul><ol><li><dl><dd><dt><table><caption><td><th><tr><thead><tbody><tfoot><col><colgroup><div><span><object><embed><param><frame><frameset><noframes><iframe>';
    // 全HTML格式
 ...
 * 增加用户积分函数
 *
 * @param string $name	积分标识名
 * @param int $lock_time 解锁时间，即多长时间内才能重复加积分，为0时不作控制
 * @param array $credit 自定义积分值，格式：array('score'=>财富值,'experience'=>经验值);为空时默认取管理中心积分管理里的配置值
 * @param int $admin_uid 管理员UID，用于管理员给用户手工加积分时的场景
 */
...
function event_url($addon_title, $id = '0') {
    ...
    if ($event) {
        ...
        $param ['id'] = $event ['id'];
        $event_url = wx_addons_url('Scratch://Scratch/show', $param);
    }
    return $event_url;
}
...
//导出数据到excel文件并下载 有样式 auth@changzhengfei 
...
function get_production_cover_url( $cover_id ){
    return D('Home/Template')->get_cover_url($cover_id);
}

// 获取redis实例
function redis($conf = array()) {
    static $redis = array();
    $redis_host = isset($conf['REDIS_HOST']) ? $conf['REDIS_HOST'] : C('REDIS_HOST');
    $redis_port = isset($conf['REDIS_PORT']) ? $conf['REDIS_PORT'] : C('REDIS_PORT');
    $connRedisUri = 'tcp://'.$redis_host.':'.$redis_port;
    if (isset($redis[$connRedisUri])) {
        return $redis[$connRedisUri];
    } else {
        ...
        $redis[$connRedisUri] = new \Predis\Client($conf);
        return $redis[$connRedisUri];
    }
}
