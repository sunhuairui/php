...
,checkURL: function (a) {
    ...
    xhr.check_login = function(fn, err_fn) {
        // use jsonp
        var check_login_url = '/index.php?s=Home/GameCreator/checklogin';
        return $.ajax({
          type: 'POST',
          url: check_login_url,
          success: function(d) {
    ...
    xhr.get_template = function(fn) {
        var get_template_url = '/index.php?s=Home/GameCreator/gamesetting/appid/' + app_id;
    ...
    xhr.check_app_name = function(app_name, fn, err_fn){
        var check_app_name_url = '/index.php?s=Home/GameCreator/checkName/id/' + app_id 
    ...
    xhr.write_file = function(filename, data, fn, err_fn) {
        var write_file_url = '/index.php?s=Home/GameCreator/write/appid/' + app_id + '/filename/' + escape(filename);
    ...
    xhr.upload = function(filename, fn, err_fn){
        ...
        var upload_url = '/index.php?s=Home/GameCreator/upload/appid/' + app_id + '&filename=' + filename;
    ...
    xhr.replace_res = function(filename, filename_from, fn, err_fn) {
        var replace_res_url = '/index.php?s=Home/GameCreator/replace/appid/' + app_id + '.html';
    ...
    xhr.get_app_libs = function(fn) {
        ...
        var get_app_libs_url = '/index.php?s=Home/GameCreator/libs';
    ...
    xhr.get_app_files = function(fn){
        var get_app_files_url = '/index.php?s=Home/GameCreator/codelist/appid/' + app_id;
    ...
    $(document).ready(function(){
        ...
        xhr.get_template(function(data) {
            ...
            for (var k in vars){
                v = vars[k] || {};
                v.name = v.name || k;
                v_rest = {name:k,staticUrl:static_url};
                v.desc = v.desc || v.name;
                
                if (v.type == 'IMAGE') {
                    html.elements.push(tmpl.render('t-image', v, v_rest));
                    ...
                    if (v.texs){
                        for (var i=0,j=v.texs.length; i<j; i++){
                            ...
                            v_rest.index = i;
                            html.elements.push(tmpl.render('t-animation', v, v_rest))
                        }
                    // single texture
                    }else{
                        html.elements.push(tmpl.render('t-image', v, v_rest))
                    ...
                    html.elements.push(tmpl.render('t-text', v, v_rest))
            ...
            function reload_app(my_url) {
                ...
                var app_url = my_url || '/app/proxy/'+(new Date()).getTime()+'/'+app_id+'/';
            ...
            // publish
            var publish_url = '/index.php?s=Home/GameCreator/publish/appid/' + app_id;
            $('.btn-play').attr('link', publish_url);
            $('.btn-play').live('click', function() {
                xhr.check_login(function() {
                    // MI.string.checkURL
                    var app_name = MI.string.trim($('.e-info .app-name').val()),
                        app_desc = MI.string.trim($('.e-info .app-desc').val());
                    ...
                    var login_url = '/index.php?s=Home/User/login';
                    alert_tip('发布或者提交游戏需要登录哦，请 <a target="_blank" class="btn btn-primary" href="' + login_url +'">立刻登录</a>',
                        $('.btn-play')[0], 3000, true);
                });
            });
            ...
            $('.e-info .app-name').live('change',function(){
            ...
            $('.e-info .app-desc').live('change', function() {
                var s = String(MI.string.trim($(this).val()));
                if (s == '') {
                    s = $(this)[0].origin;
                    $(this).val(s);
            ...
            // show template json
            $('.e-elements .list').html(html.elements.join(''));
            ...
            $('.t-animation').live('click', function(evt) {
                $('.modify-layer').remove();
                show_uploader_layer(this);
                evt.stopPropagation();
            });
            
            $('.t-text').live('click', function(evt) {
            	$('.modify-layer').remove();
                ...
                ly.find('.txt-hint').html(v.desc || v.name || '');
                ...
                ly.css({top:y+$(this).height()+15,left:x});
            ...
            $('.t-sound').live('click',function(){
                //alert('sound');
            });
            
            $('.t-code').live('click',function(){
            ...
            $('.btn-close-code-editor').live('click',function(){
            ...
            // modify-layer
            $('.modify-layer .ghost-tabs a').live('click', function(){
            ...
            $('.modify-layer .ghost-tabs-ctn .list .libs-item').live('click', function(){
            ...
            $('.modify-layer .txt-from-web').live('keyup',function(){ txt_from_web_change(this)}).live('change',function(){ txt_from_web_change(this)});
            
            $('.modify-layer .btn-ok').live('click',function(){
                var btn_ok = $(this),
                    el = $(this).parent(),
            ...
            // pick color
            $('.evo-palcenter td').live('click', function(){
            ...
            $('.modify-layer .txt-string[type=colorpicker]').live('change keyup', function(){
                $(this).parent().find('.curr-color').css('background-color',$(this).val());
            });
            
            
            $('.modify').live('click', function(){ $('.modify-layer').remove(); });     
            $('.preview .ghost-tabs .btn-reload').live('click', function(){
                write_data();
                save_code();
                setTimeout(function(){
                    reload_app();
