...
;(function($){
  var data = {}, uuid = 0;
    $.data = function(ele, name, value){    
      var id = 'uuid'+String(ele.uuid || (ele.uuid = ++uuid));
      var args = [].slice.call(arguments),len = args.length,ret = null;   
      if(!data[id]){
          data[id]={};
      }
      
      if(len<2){
        console.log('$.data 参数错误');
      }else{

          (String(value) == 'null') ? (delete data[ele.uuid][name]) : 
          (!value ? ret = data[id][name] : ret = data[id][name] = value);
      }
...
var superRE =/\b_super\b/;

function marge(target, source){
  if(target && $.isPlainObject(target) && source && $.isPlainObject(source)){
...
function createUI(name, obj){
  ...
  var Clas = $.UI[name] || Klass;
  Clas = Clas.extend(obj);
  return Clas;
}
...
function bridge(name){
  ...
  $.fn[name] = function(option){
    var obj,args = [].slice.call(arguments),ret;

     $.each(this,function(i, item){
      if(!$.data(this,'factory')){
        
        ret = $.data(item, 'factory', new $.UI[name](item, option, true));
        return false;
      }else{
        if(!args.length){
          ret = $.data(item, 'factory');
          return false;
         }else{
          var method = args[0], value = args[1],obj = $.data(item, 'factory');
          if(typeof method == 'string'){
            $.isFunction(obj[method]) ? ret = obj[method].call(obj,value) : console.log( '组件没有此方法：' + method );
          }else {
            console.log( '参数错误');
          }
         }
...
//开发及扩展UI组件对外接口
$.widgets = $.widgets || function(name, obj){
  $.UI[name] = createUI(name, obj);
  bridge(name);
}
...
BaseWidgets.prototype = {
  init:function(ele, option, fn){
    this._option = this._option || {};

    this.$ele = $(ele);
    this.evt = new Event();
    this._mergeData(option);
    marge(this._option, option);

    this.on('onBeforeCreate', this.beforeCreate);
    this.on('onAfterCreate', this.afterCreate);

    if(option.events){
      option.events.call(this, null);
    }
  ...
  on:function(name, callback, context){
    ...
    this.evt.on(name, callback, context);
  ...
  _mergeData:function(){
    var atts = this.$ele[0].attributes,
      len = atts.length,
      Reg = /^data-(\w*)?/,
      i=0,n;
    for(;i<len;i++){
      var name = atts[i].name,
        value = atts[i].value.split(" ");
      if((n = name.match(Reg))){
        this._option[n[1]] = value;
      }
      
    }
  },
  refresh:function(){
    this.$ele.find('.ui-btn-text').html(this._option.text);
  }
}
...
Event.prototype = {
  ...
  trigger:function(type, data){
    for(var i=0;i<this.events.length;i++){
      var hdl = this.events[i];
      if(hdl && hdl.type == type){
        hdl.cb.call(hdl.context, data);
      }
    }
  }
}
...
function Klass(ele, option, isInit){
...
Klass.extend = function(obj){
  ...
  initializing = true;
  var _superPro = new this();
  initializing = false;

  for(var name in obj){
    var isSuper = $.isFunction(_superPro[name])&&(superRE.test(obj[name]));

    if(isSuper){
      _superPro[name] = (function(superFn, fn){
        var _super = superFn;
        return function(){
          var old = this._super;
          this._super = _super;
      ...
      })(_superPro[name], obj[name]);
    }else{
      _superPro[name] = obj[name];      
    }
  }

  function k(ele, options){
    !initializing && this.init && this.init(ele, options)
  }

  k.prototype = _superPro;
  k.prototype.constructor = k;
  k.extend = arguments.callee;

  return  k;
}
$.extend(Klass.prototype, BaseWidgets.prototype);
...
(function($){
  ...
  var vonder = ['webkit','moz','o','MS'],
    cssPre='',
    jsPre = '',
    testEle = document.createElement('div'),
    translateZ = ' translateZ(0)';

  $.each(vonder,function(i, pre){
    if(testEle.style[pre+'TransitionProperty']!== undefined){   
      cssPre = '-'+pre+'-',jsPre = pre;
      return false;
    }
  })
  ...
  $.widgets('slider',{
    init:function(ele, option){
      var me = this;
      this._option = this._option || {};
      this._option.showButton = true;
      this._option.btnClass = 'btns';
      this._option.btnOnClass = 'action';
      this._option.isScrolling = false;
      this._option.timer = null;
      this._option.autoTime = 800;
      this._option.autoInterval = 3000;
      this._option.isTouched = false;
      this._option.isAuto = false;
      this._super(ele, option);
      ...
      this.scroller.bind('touchstart',this._handler);
      this.startAuto();
    },
    reSetData:function(){
      this.listObj = this.scroller.find("li");
      this.counts = this.listObj.length;
      this.scroller.css({width:this.counts*this.w});
    },
    beforeCreate:function(){

      this.scroller = this.$ele.find(".cont").eq(0);
      this.listObj = this.scroller.find(".item");
      var firstLi = this.listObj.eq(0);

      !this.$ele.hasClass('waper')&&this.$ele.addClass('waper');
      !this.scroller.hasClass('cont')&&this.scroller.addClass('cont');
      ...
      this.listObj.css({width:this.w});
      this.scroller.css({width:this.counts*this.w});
      this.sW = this.scroller.width();
      this.moveTo(0, 0);
      this.h = firstLi.height();
    },
    _create:function(){
      var self = this;
      if(this._option.showButton){
        this._createBtn();
      }
      this.scroller.bind(jsPre+"TransitionEnd",function(){
        self._translateEnd.apply(self, arguments)
      },false)
    ...
    _onStart:function(e){
      ...
      this.x =this.initPos.x = touch.pageX;
      this.y = touch.pageY;
      this.time  = +new Date().getTime();

      this.scroller.bind('touchmove touchend touchcancel', this._handler);



    },
    _onMove:function(e){
      var touch=e.touches[0],pos = this.getPos();
      var distX = Number(pos.x) + touch.pageX -this.x;
      var distY = Number(pos.y) + touch.pageY - this.y;
      this.x = touch.pageX;
      this.y = touch.pageY;
    ...
    _onEnd:function(e){
      ...
      this.scroller.unbind('touchmove touchend touchcancel', this._handler);
      ...
      var dir = touch.pageX > this.initPos.x ? 'r' : 'l';
      var isChange = Math.abs(touch.pageX - this.initPos.x) > 30;
      var duration = time - this.time;
      duration = duration/250<1 ? (duration/250)*1000 : 600;


      if(!touch || touch.pageX==this.initPos.x){
        return true;
      }
      ...
      this._translate(-this.index*this.w, duration);
    ...
    _createBtn:function(){
      var html=[];
      html.push("<ul class='btn "+this._option.btnClass+"'>");
      for(var i=0;i<this.counts;i++){
        html.push('<li></li>')
      }
      html.push('</ul>');
      this.btns = $(html.join(''));
      this.btns.find('li').eq(0).addClass('on');
      this.$ele.append(this.btns);
    },

    moveTo:function(x,y){
      this.scroller.css(cssPre+'transition-duration','0ms');
      this.scroller.css(cssPre+'transform','translate('+x+'px, '+y+'px)'+translateZ+';');
    },
    scrollTo:function(index){
      this.index = index;
      this._translate(-this.index*this.w, 0);
    },
    _translate:function(dist, speed){
      this.scroller.css(cssPre+'transition-duration',speed+'ms');
      this.scroller.css(cssPre+'transform','translate('+dist+'px, 0px)'+translateZ+';');
    },
    _translateEnd:function(){

      this._option.isScrolling = false;
      this.startAuto();
      if(this._option.showButton){
        this.btns.find('li').removeClass('on').eq(this.index).addClass('on');
      }
    ...
    getPos:function(){
      var RE = /\-?[0-9]+/g,
        str = this.scroller.css(cssPre+'transform'),
        pos = str.match(RE);
        return {
          x:pos[0],
          y:pos[1]
    ...
    startAuto:function(){
      ...
      if(!this._option.isAuto) {return false;}
      this._option.timer = setTimeout(function(){
        self.autoPlay();
      }, this._option.autoInterval);
    },
    autoPlay:function(){
      if(this.index == this.counts-1){
        this.index = -1;
    ...
    stopAuto:function(){
      clearTimeout(this._option.timer);
    },
    toPre:function(){
      this.index--;
      this._translate(-this.index*this.w, this._option.autoTime);
    },
    toNext:function(){
      this.index++;
      this._translate(-this.index*this.w, this._option.autoTime);
