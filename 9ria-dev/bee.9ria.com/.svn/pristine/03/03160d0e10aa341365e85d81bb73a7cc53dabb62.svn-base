...
class DealController extends WidgetController {
    ...
    function info() {
        return array(
            ...
            'set' => wx_addons_url("Diy://Diy/param", array(
                'widget' => 'Picture'
            )), // 可为空，点击确定后保存配置的地址，为空则使用通用的方法保存
            'save' => wx_addons_url("Diy://widget/saveTemplateCode"), // 可为空，保存自定义模板的地址，为空则使用通用的方法保存
            'html' => wx_addons_url("Diy://Widget/getTemplateCode")  // 可为空，获取选择的模板的html代码，为空则使用通用的方法获取
        );
    }
