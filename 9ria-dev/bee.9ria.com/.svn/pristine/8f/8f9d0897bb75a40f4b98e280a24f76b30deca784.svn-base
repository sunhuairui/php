...
class TemplateController extends BaseController {
    protected function _initialize() {
        ...
        $res['title'] = '图文列表模板';
        $res['url'] = wx_addons_url('WeiSite://template/lists');
        $res['class'] = $action == 'lists' ? 'cur' : '';
        $nav[] = $res;

        $res['title'] = '图文内容模板';
        $res['url'] = wx_addons_url('WeiSite://template/detail');
        $res['class'] = $action == 'detail' ? 'cur' : '';
        $nav[] = $res;

        $res['title'] = '底部菜单模板';
        $res['url'] = wx_addons_url('WeiSite://template/footer');
    ...
    function index() {
        // 使用提示
        $normal_tips = '点击选中下面模板即可实时切换模板，请慎重点击。选择后可点击<a target="_blank" href="' . wx_addons_url('WeiSite://WeiSite/index') . '">这里</a>进行预览';
    ...
    function _getTemplateByDir($type = 'TemplateIndex') {
        ...
        while ($file = readdir($dirObj)) {
            ...
            // 获取效果图
            if (file_exists($dir . '/' . $file . '/info.php')) {
                $res['icon'] = __ROOT__ . '/Public/Addons/' . _ADDONS .'/default/' . $type . '/' . $file . '/icon.png';
            } else {
