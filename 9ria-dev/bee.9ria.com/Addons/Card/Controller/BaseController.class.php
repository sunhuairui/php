...
class BaseController extends AddonsController {
    ...
    function _initialize() {
        ...
        $res ['title'] = '会员卡制作';
        $res ['url'] = wx_addons_url('Card://Card/config');
        $res ['class'] = $controller == 'card' ? 'current' : '';
        $nav [] = $res;

        $res ['title'] = '会员管理';
        $res ['url'] = wx_addons_url('Card://member/lists');
        $res ['class'] = $controller == 'member' ? 'current' : '';
        $nav [] = $res;

        $res ['title'] = '通知管理';
        $res ['url'] = wx_addons_url('Card://notice/lists');
        $res ['class'] = $controller == 'notice' ? 'current' : '';
        $nav [] = $res;

        $res ['title'] = '优惠券';
        $res ['url'] = wx_addons_url('Coupon://Coupon/lists');
