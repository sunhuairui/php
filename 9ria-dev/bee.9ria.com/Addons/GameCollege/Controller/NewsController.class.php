...
class NewsController extends BaseController {
    ...
    public function lists() {
        ...
        foreach ($data as &$vo) {
            $cond = array('id'=>array('in', $vo['mult_ids']));
            $orderby = "FIELD(id,{$vo['mult_ids']})";
            $list = M('custom_reply_news')->field('title')->where($cond)->order($orderby)->select();
    ...
    public function edit() {
        if (IS_POST) {
            ...
            $aim_id = intval($_GET['id']);
            $save['mult_ids'] = implode(',', $ids);
            $save['keyword'] = I('post.keyword');
            $save['keyword_type'] = I('post.keyword_type');
            M('custom_reply_mult')->where(array('id'=>$aim_id))->save($save);
            if ($save['keyword'] && !isset($_REQUEST['keyword_no_deal'])) {
                D('Common/Keyword')->set($save['keyword'], 'CustomReply', $aim_id, $save['keyword_type'], 'custom_reply_mult');
            }
        ...
        $info = M('custom_reply_mult')->where(array('id'=>intval($_GET['id'])))->find();
        $this->assign('mult', $info);
        
        $cond = array('id'=>array('in', $info['mult_ids']));
        $list = M('custom_reply_news')->where($cond)->order("FIELD(id,{$info['mult_ids']})")->select();
    ...
    public function add() {
        if (IS_POST) {
            ...
            $map_id = M('custom_reply_mult')->add($save);
            if ($save['keyword'] && !isset($_REQUEST['keyword_no_deal'])) {
                D('Common/Keyword')->set($save['keyword'], 'CustomReply', $map_id, $save['keyword_type'], 'custom_reply_mult');
            }
