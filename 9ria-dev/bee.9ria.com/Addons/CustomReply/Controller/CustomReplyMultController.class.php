...
class CustomReplyMultController extends BaseController {
    ...
    public function lists() {
        ...
        unset($map);
        foreach ($data as &$vo) {
            $map['id'] = array('in',$vo['mult_ids']);
            $list = M('custom_reply_news')->field('title')->where($map)->order("FIELD(id,{$vo['mult_ids']})")->select();
    ...
    public function edit() {
        ...
        $list = M('custom_reply_news')->where($map)->order("FIELD(id,{$info['mult_ids']})")->select();
